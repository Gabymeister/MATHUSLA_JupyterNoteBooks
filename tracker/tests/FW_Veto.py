...
eff = sys.argv[1]; layers = sys.argv[2]; noise = sys.argv[3]; CHI2 = sys.argv[4]; DIST = sys.argv[5];
if int(CHI2) == 0:
    mode = "distance"
else:
    mode = "chi2"
...
for f in pathList:
    ...
    for i in range(len(file_hits)):
        ...
        nEvents += 1
        if mode == "distance":
            if VT.FW_Veto.DistanceVeto(vertices, tracks, hits, float(DIST)):
                nVetoed += 1
        else: 
            if VT.FW_Veto.Chi2Veto(vertices, tracks, hits, float(CHI2)):
                nVetoed += 1
