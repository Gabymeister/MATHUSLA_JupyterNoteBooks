...
for f in pathList:
    ...
    for i in range(len(file_hits)):
        ...
        if len(vertices) == 0:
            continue
        nEvents +=1
        if VT.FW_Veto.Chi2Veto(vertices, tracks, hits, 20) or \
        VT.IPVeto.DistanceVeto(vertices, tracks, hits, 100) or \
        VT.UpVertexVeto(vertices, tracks) or \
        MatVet.DistanceVeto(vertices, materials, 50, False) or \
        VT.CutoffVeto(vertices, int(layers)):
            nVetoed +=1
