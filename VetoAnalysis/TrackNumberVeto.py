...
eff = sys.argv[1]; layers = sys.argv[2]; noise = sys.argv[3]; NUM = sys.argv[4]; STRICT = sys.argv[5];
if STRICT == "True":
    mode = "strict"
    STRICT = True
else:
...
print("Running TrackNumber Veto")
print("noise: " + noise)
print("layers: " + layers)
...
for f in pathList:
    ...
    for i in range(len(file_vertices)):
        ...
        if len(vertices) == 0:
            continue
        nEvents += 1
        if VT.TrackNumberVeto(vertices, int(NUM), STRICT):
                nVetoed += 1
